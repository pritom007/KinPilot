...
class VoiceAudioRoute(context: Context) {
    ...
    fun cycle(): String {
        ...
        return select(options[(options.indexOf(label).takeIf { it >= 0 } ?: 0).let { (it + 1) % options.size }])
    }
    private fun select(target: String): String {
        val types = when(target) { "Earpiece"->setOf(AudioDeviceInfo.TYPE_BUILTIN_EARPIECE); "Wired headset"->setOf(AudioDeviceInfo.TYPE_WIRED_HEADSET,AudioDeviceInfo.TYPE_WIRED_HEADPHONES); "Bluetooth"->setOf(AudioDeviceInfo.TYPE_BLUETOOTH_SCO,AudioDeviceInfo.TYPE_BLE_HEADSET); else->setOf(AudioDeviceInfo.TYPE_BUILTIN_SPEAKER) }
